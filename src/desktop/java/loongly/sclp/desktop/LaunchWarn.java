package loongly.sclp.desktop;

import javax.swing.*;

import loongly.sclp.desktop.utils.browse.BrowseUrlHandler;
...
public class LaunchWarn 
{
    ...
    {//使用String Buffer性能更好（虽然也不差这点性能）
        ...
        StringBuffer tmpOpenHelpPageMsg = new StringBuffer()
            .append("The help page has been opened. If the page cannot load, please check your network connection or try using VPN\n")
            .append("已经打开帮助页面,如果页面无法加载请检查网络连接或使用VPN后尝试\n")
            .append("ヘルプページが開きました。ページがロードできない場合はネットワーク接続を確認するか、VPNを使用してみてください\n")
            .append("La page d'aide est déjà ouverte, si la page ne se charge pas, vérifiez votre connexion réseau ou essayez après avoir utilisé un VPN\n");
        OPENED_HELP_PAGE_MESSAGE = tmpOpenHelpPageMsg.toString();
        StringBuffer tmpFallbackMsg = new StringBuffer();
            tmpFallbackMsg.append("You have tried to launch Sodium Chinese Localized Package (a Minecraft mod) directly, but it is not an executable program or mod installer. Instead, \n");
            tmpFallbackMsg.append("you must install Fabric Loader or Neoforge Loader for Minecraft, and then place this file in your mods directory.\n");
            tmpFallbackMsg.append("\n");
            tmpFallbackMsg.append("\n");
            tmpFallbackMsg.append("看起来你试图执行钠-汉化包模组，但是它并不是一个可执行程序或模组安装器。你需要安装Fabric加载器或者Neoforge加载器，然后将这个文件放到你的模组文件夹中。\n");
            tmpFallbackMsg.append("如果这是你第一次使用Fabric加载器或者Neoforge加载器安装模组，那么点击“Help”按钮以寻求帮助。\n");
            tmpFallbackMsg.append("\n");
            tmpFallbackMsg.append("\n");
            tmpFallbackMsg.append("あなたがナトリウム-中国語化パッケージモジュールを実行しようとしているように見えますが、それは実行可能プログラムやモジュールインストーラではありません。FabricローダーまたはNeoforgeローダーをインストールした後、このファイルをモジュールフォルダに入れてください。\n");
            tmpFallbackMsg.append("これがFabricローダーやNeoforgeローダーで初めてMODをインストールする場合、ヘルプボタンをクリックしてサポートを受けてください。\n");
            tmpFallbackMsg.append("\n");
            tmpFallbackMsg.append("\n");
            tmpFallbackMsg.append("Il semble que vous essayiez d'exécuter un module de sodium, mais ce n'est pas un programme exécutable ou un installateur de module. Vous devez installer Fabric loader ou neoforge Loader, puis déposer ce fichier dans votre dossier modules.\n");
            tmpFallbackMsg.append("Si c’est la première fois que vous installez un module avec Fabric loader ou neoforge Loader, appuyez sur le bouton « aide » pour obtenir de l’aide.\n");
        FALLBACK_MESSAGE = tmpFallbackMsg.toString();
    }
    ...
    {
        Image iconImg = Toolkit.getDefaultToolkit().getImage(LaunchWarn.class.getResource("/assets/sclp/icon.png"));
